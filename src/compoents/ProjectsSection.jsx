    ...
    <section id="projects" className="py-24 px-4 relative">
      <div className="container mx-auto max-w-5xl">
          ...
          {projects.map((project, key) => (
            ...
            >
              ...
              <div className="h-48 overflow-hidden">
                <img
                  src={`projects/${project.image}-${theme}.png`}
                  alt={project.title}
                  className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                  onError={(e) => {
                    e.currentTarget.src = `projects/${project.image}.png`;
                  }}
                />
              </div>
              ...
              {/* ✅ Fixed Hover Popup */}
              <div
                className="absolute left-1/2 top-1/2 z-10 w-11/12 max-w-md -translate-x-1/2 -translate-y-1/2 
                rounded-lg bg-card p-4 sm:p-6 shadow-lg border border-border
                opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto 
                transition-opacity duration-300 ease-in-out"
              >
                <h4 className="font-semibold mb-2 text-primary text-center">
                  {project.title}
                </h4>
                <p className="text-sm text-foreground max-h-48 overflow-y-auto scrollbar-none">
                  {project.description}
                </p>
              </div>
            </div>
        ...
        <div className="text-center mt-12">
          <h3 className="text-2xl font-semibold text-center text-primary mb-10">
            GitHub Projects
          </h3>
